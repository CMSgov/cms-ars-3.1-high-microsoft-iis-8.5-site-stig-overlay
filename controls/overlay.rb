...
include_controls 'microsoft-iis-8.5-site-stig-baseline' do
  ...
  control 'V-76773' do
    ...
    Expand "siteDefaults".
    Expand "limits".
    Review the results and verify the value is "1" for the "maxconnections" parameter.
    If the maxconnections parameter is set a value other than "1", this is a finding.'
  ...
  control 'V-76847' do
    title 'The IIS 8.5 websites must utilize ports, protocols, and services according to CMS guidelines.'
    ...
    The web server must provide the capability to disable or deactivate network-related services that are deemed to be non-essential to the server mission, are too unsecure, or are prohibited by CMS and vulnerability assessments.

    Failure to comply with CMS ports, protocols, and services (PPS) requirements can result in compromise of enclave boundary protections and/or functionality of the Information System.

    The ISSO will ensure web servers are configured to use only necessary ports, protocols, and services.'
    desc 'check', 'Review the website to determine if HTTP and HTTPs (e.g., 80 and 443) are used.
  ...
  control 'V-76849' do
    title 'The IIS 8.5 private website have a server certificate issued by CMS PKI or CMS-approved PKI Certification Authorities (CAs).'
    desc 'The use of a CMS PKI certificate ensures clients the private website they are connecting to is legitimate, and is an essential part of the CMS defense-in-depth strategy.'
    ...
    If the list of CAs in the trust hierarchy does not lead to the CMS PKI Root CA, CMS-approved external certificate authority (ECA), or CMS-approved external partner, this is a finding.
  ...
  control 'V-76867' do
    desc 'check', 'Note: Recycling Application Pools can create an unstable environment in a 64-bit SharePoint environment. If operational issues arise, with supporting documentation from the ISSO, this check can be downgraded to a Low Impact/Severity.
  ...
  control 'V-76869' do
    desc 'check', 'Note: Recycling Application Pools can create an unstable environment in a 64-bit SharePoint environment. If operational issues arise, mitigation steps can be set, to include setting the Fixed number or requests, Specific time, and Private memory usage in the recycling conditions lieu of the Virtual memory setting. If mitigation is used in lieu of this requirement, with supporting documentation from the ISSO, this check can be downgraded to a Low Impact/Severity.
  ...
  control 'V-76891' do
    title 'The required CMS banner page must be displayed to authenticated users accessing a CMS private website.'
    desc 'A consent banner will be in place to make prospective entrants aware that the website they are about to enter is a CMS web site and their activity is subject to monitoring.

    The requirement for the banner is for websites with security and access controls. These are restricted and not publicly accessible. If the website does not require authentication/authorization for use, then the banner does not need to be present. A manual check of the document root directory for a banner page file (such as banner.html) or navigation to the website via a browser can be used to confirm the information provided from interviewing the web staff.'
    ...
    * By using this system, you understand and consent to the following:   
    ...
    desc 'fix', 'Configure a CMS private website to display the required CMS banner page when authentication is required for user access.'
    describe 'Manual review of website is needed' do
      skip "Manual review that required CMS banner page is displayed to authenticated users accessing a CMS private website"
    end
  end
